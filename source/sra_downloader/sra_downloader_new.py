...
def download_SRA(sra_accession):
    ...
    output_dir = "/tmp/download_" + sra_accession
    try:
        os.mkdir(output_dir)
    except:
    ...
    command = "{}/prefetch -O {} {}".format(APPLICATION_FOLDER, output_dir, sra_accession)
    prefetch_proc = Popen(shlex.split(command), stdout=PIPE, stderr=PIPE)
    prefetch_out, prefetch_err = prefetch_proc.communicate()
    sra_file = "{}/{}.sra".format(output_dir, sra_accession)

    if prefetch_proc.returncode != 0 or not os.path.exists(sra_file):
        raise ValueError("prefetch failed!\nstdout:{}\nstderr:{}".format(prefetch_out, prefetch_err))

    command = "{}/fastq-dump --gzip --split-files -O {} {}".format(APPLICATION_FOLDER, output_dir, sra_file)
    fastq_dump_proc = Popen(shlex.split(command), stdout=PIPE, stderr=PIPE)
    fastq_dump_out, fastq_dump_err = fastq_dump_proc.communicate()
    os.remove(sra_file)
    file_list = [f for f in os.listdir(output_dir) if f.startswith(sra_accession)]

    if fastq_dump_proc.returncode != 0 or not (0 < len(file_list) < 3):
        raise ValueError("fastq-dump failed!\nstdout:{}\nstderr:{}".format(fastq_dump_out, fastq_dump_err))
